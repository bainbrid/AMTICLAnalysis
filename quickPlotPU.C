...
std::vector<myPlot1D> makeAllPlots(){
  ...
  std::string fileName = "plotList.dat";
...
int getTree(std::string filePath,
  ...
  friendTree->AddFile((filePath+"/Truth/"+iters[0][0]+"/"+pteta+"/"+(isDebug?"Debug/":"")+"/TruthTree_"+regShort[iRegion]+".root").c_str());
  ...
  friendTree3->AddFile((filePath+"/Reco/"+pteta+"/"+(isDebug?"Debug/":"")+"/RecoTree_"+regShort[iRegion]+".root").c_str());
